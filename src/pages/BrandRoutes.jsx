  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={<BrandPerformanceCampaignPage />}
      />
...
export const CampaignDetailPage = () => {
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`${config.BACKEND_URL}/brand/performance/bulk/${id}`, {
      headers: {
  ...
  const handleExportClick = () => {
    const csv = Papa.unparse(
      performanceData.map((item) => ({
        ...
        "Engagement Rate":
          item.views > 0
            ? `${(((item.likes + item.comments) / item.views) * 100).toFixed(
                2
              )}%`
            : "0.00%",
  ...
  const handleScrape = (item) => {
    fetch(`${config.BACKEND_URL}/brand/performance/scrape`, {
      method: "POST",
      headers: {
    ...
    <div className="p-4">
      ...
      {performanceData.length === 0 ? (
        <div className="bg-gray-100 p-4 rounded text-center">
          <p className="text-xl">
            📭 No content has been submitted by creators for this campaign yet.
          </p>
        </div>
      ) : (
            ...
            {performanceData.map((item) => (
              <tr key={item.content_url}>
                ...
                <td className="border px-4 py-2">
                  {item.views > 0
                    ? (
                        ((item.likes + item.comments) / item.views) *
                        100
                      ).toFixed(2)
                    : "0.00"}
                  %
