...
const ContentAnalysisPanel = () => {
  ...
  const fetchExistingAnalysis = async () => {
    try {
      ...
      const response = await axios.get('/api/user/content-analysis', {
  ...
  const checkConnectedPlatforms = async () => {
    try {
      const response = await axios.get('/api/user/content-analysis/social-connections', {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const runAnalysis = async (platform) => {
    try {
      ...
      const endpoint = platform === 'all' 
        ? '/api/user/content-analysis/all'
        : `/api/user/content-analysis/${platform}`;
      ...
      toast.error(error.response?.data?.message || `Failed to analyze ${platform} content`);
    } finally {
  ...
  const clearAnalysis = async () => {
    try {
      const response = await axios.delete('/api/user/content-analysis', {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border p-6">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Analyze your social media content to improve campaign matching and recommendations
          </p>
        </div>
        {analysis && (
          <button
      ...
      <div className="mb-6">
        <h4 className="text-sm font-medium text-gray-700 mb-3">Connected Platforms</h4>
        ...
        <div className="mb-6">
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 mb-4">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-sm font-medium text-gray-700">Overall Content Score</h4>
              <span className="text-2xl font-bold text-indigo-600">{analysis.overallScore}/100</span>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            <div>
              <h5 className="text-sm font-medium text-gray-700 mb-2">Content Categories</h5>
              <div className="flex flex-wrap gap-1">
                {analysis.categories.length > 0 ? analysis.categories.map((category, index) => (
                  <span
                    key={index}
                  ...
                  <span className="text-sm text-gray-500">No categories detected</span>
                )}
              </div>
            </div>
            
            <div>
              <h5 className="text-sm font-medium text-gray-700 mb-2">Top Tags</h5>
              <div className="flex flex-wrap gap-1">
                {analysis.tags.length > 0 ? analysis.tags.slice(0, 10).map((tag, index) => (
                  <span
                    key={index}
                  ...
                  <span className="text-sm text-gray-500">No tags detected</span>
                )}
              </div>
            </div>
          </div>
          ...
          {analysis.platforms.length > 0 && (
            <div className="mb-4">
              <h5 className="text-sm font-medium text-gray-700 mb-3">Platform Analysis</h5>
              <div className="space-y-2">
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          disabled={(!connectedPlatforms.instagram && !connectedPlatforms.tiktok) || loading.all}
        ...
        >
          {loading.all ? 'Analyzing All Platforms...' : 'Analyze All Connected Platforms'}
        </button>
      </div>
