...
export default function CampaignFormWizard({
  setShowModal,
  campaignData,
  index,
}) {
  ...
  const [campaign, setCampaign] = useState({
    ...
    category: campaignData?.category || "", // (e.g. "Beauty")
    brandName: campaignData?.brandName || "",
    productName: campaignData?.productName || "",
    productDescription: campaignData?.productDescription || "",
    brandLogo: campaignData?.brandLogo || null, // File object
    productImages: campaignData?.productImages || [], // Array<File>
    numCreators: campaignData?.creatorCount || campaignData?.recruiting || 10, // 10 / 15 / 20
    recruiting: campaignData?.recruiting || campaignData?.creatorCount || 10, // 10 / 15 / 20
    // (We'll compute "applicantsToShow" on the backend from this)

    // 🟠 Step 1 – Pricing Model + Content Request
    pricingModel: campaignData?.pricingModel || "", // "fixed" or "bidding"
    fixedPrice: campaignData?.price || "", // if pricingModel === "fixed"
    minBid: campaignData?.minBid || "", // if pricingModel === "bidding"
    maxBid: campaignData?.maxBid || "", // if pricingModel === "bidding"
    requestedContent: {
      videos: campaignData?.requestedContent?.videos || "",
      photos: campaignData?.requestedContent?.photos || "",
      notes: campaignData?.requestedContent?.notes || "",
    },

    // Step 2 (Content Guidelines)
    contentPlatform: campaignData?.contentPlatform || [], // "TikTok" / "Instagram" / "YouTube"
    requiredHashtags: campaignData?.requiredHashtags || [], // ["#skincare", "#glow"]
    mentionHandle: campaignData.mentionHandle || "", // "@matchably.official"
    toneMessage: campaignData.toneMessage || "", // e.g. "Bright, positive tone"
    referenceContent: campaignData.referenceContent || "", // URL or optional
    referenceFile: campaignData?.referenceFile || null, // File object (optional)
    ...
    influencersReceive: campaignData?.influencersReceive || "",
    participationRequirements: campaignData?.participationRequirements || "",
    deadline: campaignData?.deadline || "", // or null, depending how you handle it
    recruitmentEndDate: campaignData?.recruitmentEndDate || "", // likewise
  });
  ...
  useEffect(() => {
    ...
    setCampaign({
      ...
      productImages: Array.isArray(selected.productImages)
        ...
        : [],
      numCreators:
        selected.creatorCount != null
          ? String(selected.creatorCount)
          : selected.recruiting != null
          ? String(selected.recruiting)
          : "10", // ensure it's a string
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (campaign?.referenceContent) {
        console.log(campaign?.referenceContent);
        referenceUrls.push(campaign.referenceContent);
      }
      ...
      const payload = {
        ...
        locationFilter: {
          ...
          isGlobal:
            campaign.locationFilter?.isGlobal ??
            campaign.locationFilter?.locations?.length === 0,
        },
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-white">
              Edit Campaign : {campaignData.campaignTitle || ""}
            </h2>
            <button
