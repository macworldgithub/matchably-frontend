...
import AIGenerateCampaign from "../aiGenerateCampaign";
...
// Country options for creator preferences
const getCountryOptions = () => countryList().getData();
...
export default function CampaignFormWizard({ setShowModal, onCampaignAdded }) {
  ...
  // Country selection state for creator requirements
  const [selectedCreatorCountries, setSelectedCreatorCountries] = useState([]);
  ...
  const [campaign, setCampaign] = useState({
    ...
    creatorRequirements: {
      ...
      preferredCountries: [], // Array of selected countries
      minAge: "",
      maxAge: "",
      gender: "Any",
    },
  ...
  // ───────────────────────────────────────────────────
  // HANDLE CREATOR COUNTRY PREFERENCES
  // ───────────────────────────────────────────────────
  const handleCreatorCountriesChange = (selectedOptions) => {
    const countries = selectedOptions
      ? selectedOptions.map((option) => option.value)
      : [];
    setSelectedCreatorCountries(selectedOptions || []);
    setCampaign((prev) => ({
      ...prev,
      creatorRequirements: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        toneGuide: campaign?.toneMessage,
        referenceContent: referenceUrls,

        // Creator Requirements
        creatorRequirements: {
          followerCount: campaign.creatorRequirements.followerCount,
          preferredCountries: campaign.creatorRequirements.preferredCountries,
        ...
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex justify-between items-center w-full mr-5">
              ...
              <div>
                <AIGenerateCampaign setCampaign={setCampaign} />
              </div>
            </div>
            <button
              ...
              <div>
                ...
                <div className="mt-8 border-t border-gray-800 pt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Location Filter - spanning full width */}
                    ...
                    {/* Preferred Creator Countries */}
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-300 mb-2">
                        Preferred Creator Countries (Optional)
                      </label>
                      <Select
                        isMulti
                        options={getCountryOptions()}
                        value={selectedCreatorCountries}
                        onChange={handleCreatorCountriesChange}
                        placeholder="Select countries for creator preferences..."
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        This is in addition to the location targeting above.
                        Select specific countries where you prefer creators to
                        be located. Leave empty for no preference.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {step === 4 && (
              <div>
                ...
                <div className="space-y-6 text-gray-200">
                  ...
                  {/* Creator Requirements Summary */}
                  <div className="bg-gray-800 p-4 rounded-lg">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-semibold">Creator Requirements</h4>
                      <button
                        type="button"
                        onClick={() => setStep(2)}
                        className="text-blue-400 hover:text-blue-300 text-sm"
                    ...
                    <div className="space-y-2">
                      <p>
                        ...
                        {campaign.creatorRequirements.followerCount}
                      </p>

                      {campaign.creatorRequirements.minAge && (
                        <p>
                          <span className="font-semibold">Age Range:</span>{" "}
                          {campaign.creatorRequirements.minAge}
                          {campaign.creatorRequirements.maxAge &&
                            ` - ${campaign.creatorRequirements.maxAge}`}{" "}
                          years old
                        </p>
                      )}
                      ...
                      <p>
                        ...
                        {campaign.creatorRequirements.gender}
                      </p>

                      {campaign.creatorRequirements.preferredCountries?.length >
                        0 && (
                        <p>
                          <span className="font-semibold">
                            Preferred Countries:
                          </span>{" "}
                          {selectedCreatorCountries
                      ...
                      {(campaign.locationFilter.locations?.length > 0 ||
                        ...
                        <p>
                          ...
                          {campaign.locationFilter.isGlobal ||
                          campaign.locationFilter.locations?.some(
                            (loc) => loc.value === "global"
                          )
                            ? "Worldwide (Global)"
                            : campaign.locationFilter.locations
                                ?.map((loc) => loc.label)
                                .join(", ") || "Global"}
                        </p>
                      )}
